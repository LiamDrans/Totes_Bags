...
def fetch_one_table_updates(
    table_name: str, conn: Optional[Connection] = None
) -> Union[Dict, bool]:
    """fetches the updates to the table ONLY with the parameter of it being last_updated 30 minutes before this code is executed and formats it
    Arguments:
        table_name (str): name of the table to query
        conn: the database connection
    ...
    """
    if rows := query_db(
        f"SELECT * FROM {identifier(table_name)} WHERE last_updated >= NOW() - interval '30 minutes';",
        conn,
    ):
        return format_response(conn.columns, rows, label=table_name)
    return False
...
def fetch_all_tables(updates=False) -> List | bool:
    ...
    with CreateConnection() as conn:
        try:
            ...
            for name in table_names:
                if updates:
                    table_data = fetch_one_table_updates(name, conn)
                else:
