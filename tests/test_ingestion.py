...
from db.ingestion import lambda_handler
...
def test_lambda_handler_upload(MockConnection, postgresql):
    ...
    assert json.load(result["Body"]) == [{'test': [{'id': 1, 'num': 1, 'last_updated': 'Thu, 03 Nov 2022 14:20:49 GMT'}]}, {'test2': [{'id': 1, 'num': 2, 'last_updated': 'Thu, 03 Nov 2022 14:20:49 GMT'}]}]
...
def test_lambda_handler_update(MockConnection, postgresql):
    ...
    test_data = [{'test': [{'id': 1, 'num': 1, 'last_updated': 'Thu, 03 Nov 2022 14:20:49 GMT'}]}, {'test2': [{'id': 1, 'num': 2, 'last_updated': 'Thu, 03 Nov 2022 14:20:49 GMT'}]}]
    s3 = boto3.client('s3', region_name='us-east-1')
    s3.create_bucket(Bucket='totes-data-1234124')
    s3.put_object(Body= save_json(test_data),Bucket='totes-data-1234124', Key='full_db_totes.json')
